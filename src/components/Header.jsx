    ...
    <header className="fixed top-0 left-[280px] right-0 h-[64px] bg-surface/90 backdrop-blur-xl border-b border-outline-variant/10 z-40 px-6 flex items-center justify-between select-none">
      ...
      <div className="flex-1 max-w-lg">
        <div className="relative flex items-center">
          ...
          <input
            ...
            placeholder="Search fleet ID, driver name, VIN or location..."
      ...
      <div className="flex items-center gap-4 ml-6">
        ...
        <button
          ...
          title="Toggle live telemetry and GPS positioning simulation"
        >
          {isSimulating ? (
            <>
              <Pause className="w-3.5 h-3.5 text-primary" />
              <span className="font-semibold">LIVE TICKER ON</span>
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-3 w-80 bg-surface-container/95 backdrop-blur-2xl border border-outline-variant/30 rounded-2xl shadow-2xl p-4 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="flex items-center justify-between pb-3 border-b border-outline-variant/20">
                <div className="flex items-center gap-2">
                  <AlertTriangle className="w-4 h-4 text-tertiary" />
                  <span className="text-[13px] font-bold text-on-surface">Recent Incidents</span>
                </div>
                <button
                  onClick={() => {
                ...
                {alerts.slice(0, 3).map((alt) => (
                  ...
                  >
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="font-mono font-bold text-primary">{alt.unitId}</span>
                      <span className="text-on-surface-variant font-mono">{alt.timestamp}</span>
                    </div>
                    <div className="text-[12px] font-medium text-on-surface mt-1 line-clamp-1">{alt.title}</div>
                    <div className="text-[11px] text-on-surface-variant/80 mt-0.5">{alt.location}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 pl-2 border-l border-outline-variant/20">
          <div className="text-right hidden sm:block">
            <div className="text-[13px] font-semibold text-on-surface leading-tight">Fleet Admin</div>
            <div className="text-[11px] text-on-surface-variant font-mono">Chicago HQ • Ops 01</div>
          </div>
